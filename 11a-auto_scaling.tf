...
resource "aws_appautoscaling_target" "my-as-target1" {
  service_namespace               = "ecs"
  resource_id                     = "service/${aws_ecs_cluster.my-ecs-cluster.name}/${aws_ecs_service.my-ecs-service1.name}"
...
resource "aws_appautoscaling_policy" "my-as-policy-up" {
  ...
  depends_on = [aws_appautoscaling_target.my-as-target1]
}
